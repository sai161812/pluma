...
|----------|-------------------|
| [`PLUMA_MASTER_SPEC.md`](file:///c:/Workspace/DEVEL/PLUMA/PLUMA_MASTER_SPEC.md) | **Authoritative product and engineering specification.** Read before making architectural changes. |
| [`AGENTS.md`](file:///c:/Workspace/DEVEL/PLUMA/AGENTS.md) | **Mandatory safety and architecture contract** binding all coding agents. |
| [`PLUMA_BUILD_PLAN.md`](file:///c:/Workspace/DEVEL/PLUMA/PLUMA_BUILD_PLAN.md) | Ordered implementation phases (Phase 0 through Phase 14). |
| [`PLUMA_ACCEPTANCE_TESTS.md`](file:///c:/Workspace/DEVEL/PLUMA/PLUMA_ACCEPTANCE_TESTS.md) | Objective release gates and verification criteria. |
| [`PLUMA_TECH_STACK.md`](file:///c:/Workspace/DEVEL/PLUMA/PLUMA_TECH_STACK.md) | Approved runtime libraries, adapters, and technology stack. |
...
- **Automation Adapters** (stubs/planned): `pywinauto` (UIA), `ctypes` `user32`/`kernel32` (Native Win32), `subprocess` with Job Objects (PowerShell/CLI).
...
| **Phase 4** | Windows Automation Adapters (Native Win32, PowerShell, UIA, Input, Screen capture) | **NEXT UP** | Pending |
| **Phase 5** | Activity Ledger completion, redaction engine, reverse-order rollback | Planned | Pending |
...
## 5. Current Verified Implementation Details (Phases 0–3)

### Phase 0: Schema Contracts & Storage Foundation
- [`pluma/brain/schemas.py`](file:///c:/Workspace/DEVEL/PLUMA/pluma/brain/schemas.py): `RouteMode` (`FAST`, `SCREEN`, `SMART`, `DEEP`), `PlanMode`, `ToolCall`, `Plan` with hard cap step validation ($N \le 20$).
- [`pluma/core/request.py`](file:///c:/Workspace/DEVEL/PLUMA/pluma/core/request.py): `PlumaRequest`, `InputMode` (`TEXT`, `VOICE`), `RequestID` validation.
- [`pluma/core/cancellation.py`](file:///c:/Workspace/DEVEL/PLUMA/pluma/core/cancellation.py): `CancellationToken`, `StopReason`, `TaskCancelledError`.
- [`pluma/memory/db.py`](file:///c:/Workspace/DEVEL/PLUMA/pluma/memory/db.py): SQLite with WAL mode, busy timeout, queued async background writer loop for crash resiliency.
- [`pluma/config/loader.py`](file:///c:/Workspace/DEVEL/PLUMA/pluma/config/loader.py): Structured YAML config loader.
- [`tests/fixtures/golden_commands.yaml`](file:///c:/Workspace/DEVEL/PLUMA/tests/fixtures/golden_commands.yaml): 22 reference golden commands (GC-001 to GC-022) with expected routes, tools, and `no_llm`/`no_ocr` assertions.

### Phase 1: Resident Core & Process Containment
- [`pluma/core/job_object.py`](file:///c:/Workspace/DEVEL/PLUMA/pluma/core/job_object.py): Win32 `CreateJobObjectW`, `SetInformationJobObject` setting `JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE`, `AssignProcessToJobObject`.
- [`pluma/core/ownership.py`](file:///c:/Workspace/DEVEL/PLUMA/pluma/core/ownership.py): `OwnershipRegistry` tracking PID creation timestamps via `GetProcessTimes` to protect against PID reuse.
- [`pluma/core/task_supervisor.py`](file:///c:/Workspace/DEVEL/PLUMA/pluma/core/task_supervisor.py): `TaskCapsule` and `TaskSupervisor` implementing the deterministic STOP sequence (§12.2): (1) cancel latch, (2) STOPPING transition, (3) Job Object termination, (4) resource cleanup, (5) STOPPED state.
- [`pluma/core/ipc.py`](file:///c:/Workspace/DEVEL/PLUMA/pluma/core/ipc.py): Local-only Windows named pipe `IpcServer` / `IpcClient` with blocking accept loop and test isolation.
- [`pluma/core/resident.py`](file:///c:/Workspace/DEVEL/PLUMA/pluma/core/resident.py): Resident daemon with Win32 `RegisterHotKey` loop and crash recovery stubs.

### Phase 2: Typed Tool Framework & Initial Tools
- [`pluma/tools/base.py`](file:///c:/Workspace/DEVEL/PLUMA/pluma/tools/base.py): `ToolSpec`, `ToolResult`, `VerifyResult`, `RiskClass`, `AdapterPriority`.
- [`pluma/tools/registry.py`](file:///c:/Workspace/DEVEL/PLUMA/pluma/tools/registry.py): `ToolRegistry` with full `execute()` lifecycle: argument validation $\rightarrow$ cancellation latch check $\rightarrow$ undo pre-state capture $\rightarrow$ execution with perf timing $\rightarrow$ postcondition verification $\rightarrow$ Activity Ledger recording.
- [`pluma/verify/common.py`](file:///c:/Workspace/DEVEL/PLUMA/pluma/verify/common.py): Deterministic postcondition verifiers (`verify_file_exists`, `verify_file_moved`, `verify_file_renamed`, `verify_dir_created`, `verify_process_running`, `verify_process_closed`, `verify_window_focused`, `verify_noop`).
- Built-in Tool Implementations:
  - [`pluma/tools/files.py`](file:///c:/Workspace/DEVEL/PLUMA/pluma/tools/files.py): `list_files`, `find_file`, `move_file`, `rename_file`, `create_folder`.
  - [`pluma/tools/apps.py`](file:///c:/Workspace/DEVEL/PLUMA/pluma/tools/apps.py): `open_app`, `close_app`, `focus_app`, `list_apps`, `app_status` (assigned to Windows Job Objects).
  - [`pluma/tools/windows.py`](file:///c:/Workspace/DEVEL/PLUMA/pluma/tools/windows.py): `list_windows`, `focus_window`.
  - [`pluma/tools/audio.py`](file:///c:/Workspace/DEVEL/PLUMA/pluma/tools/audio.py): `set_volume`, `mute`, `unmute` (lazy `pycaw` with mock fallback for headless test environments).
  - [`pluma/tools/system.py`](file:///c:/Workspace/DEVEL/PLUMA/pluma/tools/system.py): `get_system_status`, `stop_current`, `show_activity`, `undo_last`.

### Phase 3: Deterministic FAST Route
- [`pluma/core/router.py`](file:///c:/Workspace/DEVEL/PLUMA/pluma/core/router.py):
...
- [`pluma/core/orchestrator.py`](file:///c:/Workspace/DEVEL/PLUMA/pluma/core/orchestrator.py):
...
- Additional Tools Added:
  - [`pluma/tools/clipboard.py`](file:///c:/Workspace/DEVEL/PLUMA/pluma/tools/clipboard.py): `clear_clipboard`, `clipboard_clear`, `get_clipboard_text`, `set_clipboard_text` via Win32 API with 64-bit safe `c_size_t` `HGLOBAL` types. Raw text is omitted from logged `data` for privacy.
  - [`pluma/tools/windows.py`](file:///c:/Workspace/DEVEL/PLUMA/pluma/tools/windows.py): `minimize_window`, `maximize_window` via Win32 `ShowWindow` (`SW_MINIMIZE=6`, `SW_MAXIMIZE=3`).
  - [`pluma/tools/system.py`](file:///c:/Workspace/DEVEL/PLUMA/pluma/tools/system.py): `battery_status` via Win32 `GetSystemPowerStatus`.
...
**Current status: 220 passed, 0 failed, 0 warnings (Execution time ~8.2s)**
...
2. **IPC Named Pipe Windows Behavior**: `multiprocessing.connection.wait()` does not support `Listener` objects on Windows. `IpcServer` uses a blocking `.accept()` loop in a dedicated thread.
3. **SQLite Thread Affinity**: Background writer connection must be opened inside the writer worker thread. In-memory databases (`:memory:`) share one connection object.
4. **Audio Headless/CI Mock**: `pluma/tools/audio.py` lazily attempts to load `pycaw`, falling back to an in-memory `_MOCK_AUDIO_STATE` dictionary if audio hardware is absent.
5. **PID String vs Int Compatibility**: Verifiers (`pluma/verify/common.py`) coerce numeric string PIDs to integers to prevent type mismatch between tool callers and process checkers.
6. **Task Completion Calls**: On `TaskSupervisor`, use `mark_succeeded(task_id)` and `mark_failed(task_id)` (not `complete_task`). For stops, default reason is `StopReason.USER_STOP`.

---

## 8. Current Objective & Exact Next Steps

### Next Phase: **Phase 4 — Windows Automation Adapters**
Reference: `PLUMA_BUILD_PLAN.md` Phase 4 & `PLUMA_MASTER_SPEC.md` §11, §14.

### Objectives for Phase 4:
1. **Implement Concrete Adapter Layer (`pluma/adapters/`)**:
   - `pluma/adapters/win32.py`: Concrete Win32 adapter for native window queries, focus, message posting, process inspection.
   - `pluma/adapters/powershell.py`: Controlled PowerShell execution adapter (bounded timeout, Job Object assigned, strict command whitelist/sanitization).
   - `pluma/adapters/uia.py`: UI Automation adapter (lazy pywinauto / Windows UI Automation COM, element tree lookup, control patterns).
   - `pluma/adapters/input.py`: Low-level input adapter (SendInput ctypes wrapper for keyboard keystrokes and bounded mouse actions).
   - `pluma/adapters/screen.py`: Targeted window/region screen capture adapter (bounding rect capture, zero persistent screenshots).
2. **Adapter Priority & Fallback Contracts**:
   - Implement common `AdapterInterface` / base contract.
   - Standardize error mapping (Access Denied, Element Not Found, Timeout $\rightarrow$ typed factual errors).
3. **Write Unit & Integration Tests**:
   - `tests/unit/test_adapters_win32.py`
   - `tests/unit/test_adapters_powershell.py`
   - `tests/unit/test_adapters_uia.py`
   - `tests/unit/test_adapters_input.py`
   - `tests/unit/test_adapters_screen.py`
4. **Phase 4 Gate Verification**:
   - Adapters cleanly swap behind interfaces.
   - Access-denied and unavailable-control scenarios produce deterministic factual failure messages.
   - Zero ML or OCR modules loaded by adapters.

### Exact Instructions for the Next Agent:
1. Review `PLUMA_BUILD_PLAN.md` (Phase 4 section) and `PLUMA_MASTER_SPEC.md` (§11, §14).
2. Prepare and present the Phase 4 Implementation Plan to the user for approval.
3. Upon approval, implement `pluma/adapters/` components and their corresponding unit tests.
4. Verify all tests pass (`pytest tests/unit/ -v`).
5. Update `PROJECT_HANDOFF.md` to record Phase 4 completion before proceeding to Phase 5.
