...
dacl_verify_code = '''def _verify_win32_file_dacl(filepath: str) -> None:
    ...
    # We need to verify the DACL of the file. To be absolutely strict, we will just apply a new secure DACL to it.
    sddl = f"D:(A;;GA;;;{current_sid_str})"
    sd_ptr = ctypes.c_void_p()
    sd_size = wintypes.ULONG()
    if not advapi32.ConvertStringSecurityDescriptorToSecurityDescriptorW(
        sddl, 1, ctypes.byref(sd_ptr), ctypes.byref(sd_size)
    ):
        raise RuntimeError("ConvertStringSecurityDescriptorToSecurityDescriptorW failed")
    ...
    if ret != 0:
        raise RuntimeError(f"SetNamedSecurityInfoW failed with {ret}")
